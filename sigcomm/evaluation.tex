...
7060CX-32S switch with 32 100GbE ports and 16MB packet buffer. The switches
...
to H1 and the flow from H5 to H15 take -bounce paths. This creates CBD as
discussed earlier.
...
\sysname{} requires only three classes for a network with 2000 switches, even
...
random nature of the topology.} 98 match-action rules per switch.  Modern
commodity switches can support 1-4K rules, so this is not an issue. 

We also considered smaller (50 switches, 32 ports) Jellyfish topologies with
upto 16-shortest path routing. We need just 3 lossless priorities, and no more
