...
TCAM rule list, acting as a default safeguard that avoids unexpected buffer dependency.
...
\subsection{Generating Baseline $G(V,E)$} 
\begin{figure*}[t]
	\centering
...
\begin{table*}[t]
	...
	\begin{tabular}{lll}
		...
		\end{tabular}
	\end{tabular}
	\caption{Tag rewriting rules under Algorithm~\ref{alg:ttl} (tag ``3'' will only appear on destination servers).}
...
(i.e. lossless priorities) -- in fact, as many as \fixme{the longest path length in 
lossless routes}~\cite{xxx}. This is why we need three lossless priorities for the 
simple example in Figure~\ref{fig:three_node}.  \fixme{A three-layer Clos 
network would require five priorities just for the up-down routing.}
We now show how to combine tags to reduce the
...
ensure the monotonic property, we start from combing the nodes with smallest tag, 0,
to largest tag in the brute-force tagging system. 
...
\fixme{The new tag $t'$ also starts from 0. In every iteration, we check
all nodes with the same tag value $t$. $V_{tmp}$ and $E_{tmp}$ is the 
``sandbox''. For every node, we add it to $V_{tmp}$ and $E_{tmp}$ and check 
whether adding it to $G'_{t'}$ will lead to a cycle within $G'_{t'}$. If not, 
we re-tag the node to be $t'$. Otherwise, we re-tag the node to be $t'+1$. 
\footnote{We are sure that re-tag the node to be $t'+1$ would not cause
a cycle in $G'_{t'+1}$, because all nodes in $G'_{t'+1}$ so far have the same
old tag of $t$, which means there is no edge between them.}
In the end of each iteration, if there are nodes being re-tagged as $t'+1$,
we move on to add nodes into $G'_{t'+1}$ in the next interation.
This ensures that the monotonic property will still hold after combination.}
...
and when the packet is coming down. \fixme{For example, as shown in Figure~\ref{fig:nonoptimal},
the bounce of green flow will force the algorithm to create a new tag subspace for the 
first two hops, since the third hop, which is a bouncing hop, may lead to CBD. 
However, the orange flow bounces at the last two hops and will force the algorithm to create
another new tag subspace. Adding the part in the middle, the algorithm outputs three
lossless queues.}
