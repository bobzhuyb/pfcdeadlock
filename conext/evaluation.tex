...
\textcolor{blue}{\textbf{Impact of using lossy queue as a last resort:} In Figure~\ref{fig:bounce_probability}, we measured the percentage of 
...
\textcolor{blue}{Figure~\ref{fig:bounce_probability}(a), (b) and (c) show the percentage of flows bounced once, twice and more than twice, 
...
\textcolor{blue}{In Table~\ref{table:loss_rate_ws} and Table~\ref{table:loss_rate_dm}, we evaluate the packet loss rate of \sysname{} 
under stressful traffic using NS-3 simulations. we choose the setting of FatTree(k=8) with $20\%$ link failure rate.
...
The flows are generated according to data mining~\cite{vl2} and web search~\cite{dctcp} workload with 0.99 average load on bottleneck link. 
At switches, we configure WRR scheduling among lossless and lossy queues with equal weight.}
